...
arizona_usda_path <- "Raw Data/Arizona/USDA_Comparison/Arizona_County_USDA.xlsx"
...
combined_data <- Arizona_aggregated %>%
  inner_join(Arizona_USDA, by = c("LOCALITY", "YEAR", "MONTH"))
...
plot1 <- ggplot(combined_data_long, aes(x = YEAR_MONTH, y = Ratio, color = Variable)) +
  geom_line() +
  labs(title = "Ratios Over Time by County",
       x = "Year-Month",
       y = "Ratio",
       color = "Variable") +
  theme_minimal()
...
ggsave(filename = file.path(plot_output_path, "Ratios_Over_Time_By_County.png"), plot = plot1, width = 8, height = 6)

# Save the plot as .pdf
ggsave(filename = file.path(plot_output_path, "Ratios_Over_Time_By_County.pdf"), plot = plot1, width = 8, height = 6)
