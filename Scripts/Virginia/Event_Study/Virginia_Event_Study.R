...
virginia_summary_dta_path <- "Data Outputs/Virginia/County_Comparison/Virginia_SNAP_Summary_Comparison.dta"
plot_output_dir <- "Plots/Virginia/Event_Study"

# Read the summary .dta file for Virginia
VA <- read_dta(virginia_summary_dta_path)

# Ensure the Date column is of Date type
VA$Date <- as.Date(VA$Date)

# Define the base date
base_date <- as.Date("2014-04-01")
...
# List of weighted variables for Virginia
weighted_vars <- c("weighted_HH_PA", "weighted_HH_NPA", "weighted_HH_TOTAL", 
                   "weighted_IND_PA", "weighted_IND_NPA", "weighted_IND_TOTAL", 
                   "weighted_ISS_PA", "weighted_ISS_NPA", "weighted_ISS_TOTAL")
...
plots_A <- lapply(weighted_vars, function(variable) {
  plot <- generate_plots(VA, "TREATMENT_A", "CONTROL_A", variable, "LEE COUNTY", "SCOTT COUNTY, WISE COUNTY, NORTON CITY")
  filename <- file.path(plot_output_dir, paste0(variable, "_TREATMENT_A_vs_CONTROL_A_Virginia.png"))
...
plots_B <- lapply(weighted_vars, function(variable) {
  plot <- generate_plots(VA, "TREATMENT_B", "CONTROL_B", variable, "LEE COUNTY, SCOTT COUNTY, WISE COUNTY, NORTON CITY", "all other localities besides LEE COUNTY, SCOTT COUNTY, WISE COUNTY, and NORTON CITY")
  filename <- file.path(plot_output_dir, paste0(variable, "_TREATMENT_B_vs_CONTROL_B_Virginia.png"))
  ggsave(filename = filename, plot = plot, width = 10, height = 6, dpi = 300, bg = "white")
  print(paste("Saved plot to:", filename))
  return(plot)
})
